"""E13-v2 pure learned-logit candidate ranker for OMN-Net.

The original E13 added the learned score to a fixed negative observable-distance
prior. That makes an unchanged ranking ambiguous: the student may be learning
but remain too small to cross the fixed observable gaps. E13-v2 removes that
confound. Observable distance is retained as an input feature, but the final
ranking is produced only by the learned scorer.

This is still a diagnostic learner, not a reconstruction module. Stage-1 and
Stage-2 stay frozen; candidate values remain real LR-HSI-derived P_comp
residuals; GT is used only by the external Frank-Wolfe teacher during training.
"""
...
class PureLearnedSupportRanker(SparseSimplexComplementNet):
    """Rank all K recalled candidates using learned logits only."""

    def __init__(self, *args, scorer_init_std: float = 1e-3, **kwargs):
        super().__init__(*args, **kwargs)
        if scorer_init_std <= 0:
            raise ValueError("scorer_init_std must be positive")
        # Parent CandidateScorer deliberately zero-initializes its head for the
        # sparse-simplex reconstruction experiment. For a rankability probe that
        # would block gradients into the feature/query encoders on the first
        # step, so E13-v2 uses a tiny non-zero initialization instead.
        nn.init.normal_(
    ...
    def score_queries(
        ...
        for b in range(n):
            ...
            if rank_top_m is not None:
                ...
                observable_batches.append(
                    torch.cat(observable_chunks, dim=0)
                )
        ...
        result = {
            ...
            "learned_logit_std": torch.stack(logit_std_values).mean()
            if logit_std_values
